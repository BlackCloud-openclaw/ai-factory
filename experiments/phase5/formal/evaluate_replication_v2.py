#!/usr/bin/env python3
# experiments/phase5/formal/evaluate_replication_v2.py
"""
评估 MP2 + MP3 (强化 Prompt 版本) 的 Recognition / Integration / Utilization / Continuity
"""
...
async def main():
    ...
    raw_dir = base_dir / "reports" / "raw_location_replication_v2"
    output_dir = base_dir / "reports" / "location_replication_results_v2"
    output_dir.mkdir(parents=True, exist_ok=True)

    print("="*80)
    print("Location Gap 复制验证评估 (强化 Prompt): MP2 + MP3")
    print("="*80)
    ...
    output_path = output_dir / "replication_scores_v2.json"
    ...
    # 如果有 v1 的结果，可以加载对比
    v1_path = base_dir / "reports" / "location_replication_results" / "replication_scores.json"
    if v1_path.exists():
        ...
        for label in ["MP2", "MP3"]:
            print(f"\n{label}:")
            for cond in conditions:
                v1_items = [r for r in v1_data if r["label"] == label and r["condition"] == cond]
                v2_items = [r for r in all_results if r["label"] == label and r["condition"] == cond]
                if v1_items and v2_items:
                    v1_c = statistics.mean([i["continuity"] for i in v1_items])
                    v2_c = statistics.mean([i["continuity"] for i in v2_items])
                    delta = v2_c - v1_c
                    print(f"  {cond}: v1 C={v1_c:.2f} → v2 C={v2_c:.2f} (Δ {delta:+.2f})")
