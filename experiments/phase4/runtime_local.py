...
class NarrativeRuntime:
    ...
    def _build_segment_prompt(
        ...
        if contract.scene_spec:
            ...
            lines.append(f"  开头 → 读者应感到：{spec.emotional_arc.begin}")
            lines.append(f"  中间 → 读者应感到：{spec.emotional_arc.middle}")
            lines.append(f"  结尾 → 读者应感到：{spec.emotional_arc.end}")
            lines.append("")

            lines.append("【📖 叙事功能】")
            lines.append(f"  场景功能：{spec.scene_function}")
    ...
    def _verify_segment(self, text: str, segment_units: List[ExecutionUnit]) -> bool:
        ...
        matched = 0
        for unit in segment_units:
            keywords = self._extract_core_keywords(unit.description)
            if any(kw in text for kw in keywords):
                matched += 1
        
        # 允许 50% 以上单元匹配即通过（原来是 100%）
        threshold = 0.5 if len(segment_units) >= 3 else 0.6
        return matched / len(segment_units) >= threshold
    ...
    def _parse_and_validate(self, text: str) -> Optional[WriterOutput]:
        ...
        if len(clean_text) > 200:
            ...
            return WriterOutput(scene_text=clean_text[:1500], events=[], foreshadowing=[])
        return None
