#!/usr/bin/env python3
"""
Phase 2 实验运行器 - 增量执行（Incremental Writer）
基于 Phase 1 的 Action + Dense 最佳配置，测试分段生成是否能提升质量
"""
...
logger = logging.getLogger("phase2_incremental")
...
def build_planner_prompt() -> str:
    """生成 Planner Prompt（固定 Action + Dense）"""
    return """
...
def build_segment_prompt(
    ...
    if previous_text:
        lines.append("【上一段结尾】")
        lines.append(previous_text[-200:])
...
class IncrementalWriter:
    ...
    async def write_scene(
        ...
        segment_units = self.split_units(units, num_segments)

        logger.info(f"拆分 {len(units)} 个执行单元为 {len(segment_units)} 段")

        # 2. 逐段生成
        for idx, units_in_segment in enumerate(segment_units):
            if not units_in_segment:
                continue

            logger.info(f"  段 {idx+1}/{len(segment_units)}: {len(units_in_segment)} 个单元")

            text, events = await self.write_segment(
                contract=contract,
                segment_units=units_in_segment,
                segment_idx=idx,
                total_segments=len(segment_units),
            )
...
async def run_experiment(config_path: Path, output_dir: Path):
    ...
    results_file = output_dir / f"results_phase2_{timestamp}.json"
    with open(results_file, "w", encoding="utf-8") as f:
        json.dump(all_results, f, indent=2, ensure_ascii=False)

    report = generate_report(all_results, groups)
    report_file = output_dir / f"report_phase2_{timestamp}.md"
...
def generate_report(results: List[Dict], groups: List[Dict]) -> str:
    lines = ["# Phase 2 实验报告（Incremental Writer）\n"]
